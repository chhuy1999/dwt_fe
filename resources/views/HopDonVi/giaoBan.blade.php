                                                    ...
                                                    <div class="col-12 mt-4">
                                                        <form id="commentForm">
                                                            <div class="d-flex align-items-center">
                                                                ...
                                                                <button type="submit" class="btn btn-outline-danger ms-3">Gửi
                                                                </button>
                                                            </div>
                                                        </form>
                                                    </div>
                                                ...
                                                <form action="/giao-ban/{{ $meeting->id }}" method="POST" enctype="multipart/form-data">
                                                    ...
                                                    <div class="upload_wrapper-items">
                                                        ...
                                                        @if ($meeting->files && count(explode(',', $meeting->files)))
                                                            <ul class="p-0" style="max-height: 134px; overflow-y: scroll; overflow-x: hidden;" id="uploaded-file-container">
                                                                Loading...
                                                            </ul>
                                                        @endif
                                                    ...
                                                    <tbody id="handle-reports">
                                                        @foreach ($handledReports as $item)
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <div type="text-nowrap d-block text-truncate" class="form-control border-0 bg-transparent" value="Giải quyết">Giải quyết
                                                                    </div>
                                                                </td>
                                                                <td>
                    ...
                    <div class="row">
                        <div class="col-6 col-md-6 d-flex flex-column justify-content-between">
                            ...
                            <div class="d-flex align-items-center  justify-content-center" style="margin-top: 16px">
                                ...
                                <button id="showImageBtn-leader" type="button" class="btn btn-outline-primary fs-6">Chèn chữ ký</button>
                                <img id="signatureImg-leader" width="100" style="display: none;" src="{{ asset('/assets/img/sign-temp.jpg') }}" />
                            </div>
                            <div class="d-flex align-items-center  justify-content-center" style="margin-top: 16px">
                        ...
                        <div class="col-6 col-md-6 d-flex flex-column justify-content-between">
                            ...
                            <div class="d-flex align-items-center  justify-content-center" style="margin-top: 16px">
                                ...
                                <button id="showImageBtn-secretary" type="button" class="btn btn-outline-primary fs-6">Chèn chữ ký</button>
                                <img id="signatureImg-secretary" width="100" style="display: none;" src="{{ asset('/assets/img/sign-temp.jpg') }}" />
                            </div>
                            <div class="d-flex align-items-center  justify-content-center" style="margin-top: 16px" >
    ...
    <div class="modal fade" id="orther" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 38%">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-danger" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#themThanhVien">Hủy
                    </button>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="col-md-4 mb-3">
                                    <input type="number" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Người đảm nhiệm" data-bs-original-title="Manday" required class="form-control" min="0" step="0.05" oninput="onInput(this)" placeholder="Manday" id="title" name="manDay">
                                </div>
...
@section('footer-script')
    ...
    <script>
        const showImageBtn = document.getElementById('showImageBtn-leader');
        const signatureImg = document.getElementById('signatureImg-leader');
        ...
        const showImageBtn1 = document.getElementById('showImageBtn-secretary');
        const signatureImg1 = document.getElementById('signatureImg-secretary');
    ...
    <script>
        ...
        //load bg for uploaded files
        const uploadedFileContainer = document.getElementById('uploaded-file-container');
        const renderListUploadedFile = async () => {
            uploadedFileContainer.innerHTML = '';
            const meeting = await getThisMeeting();
            const files = meeting.files ?? "";
            const listFiles = files.split(',');
            const validFiles = listFiles.filter(item => item !== '');
        ...
        renderListUploadedFile();
        //render unhandled report
        const unHandledReportContainer = document.getElementById('unhandled-reports');
        const renderUnHandledReports = async () => {
            const meeting = await getThisMeeting();
            const unHandledReports = meeting.reports.filter(item => item.status === 'Sent');
            unHandledReportContainer.innerHTML = '';
            unHandledReports.forEach((report, index) => {
                unHandledReportContainer.innerHTML += `
                <tr data-bs-toggle="modal" data-bs-target="#suaVanDeTonDong${report.id}" role="button">
                    <td>
                        <div class="d-flex align-items-center justify-content-center">${index + 1}</div>
                    </td>
                    <td>
                    ...
                    <td>${moment(report.deadline).format("DD/MM")}</td>
                    <td>
        ...
        //render handled report
        renderUnHandledReports();

        const handledReportContainer = document.getElementById('handle-reports');
        ...
        const renderHandledReports = async () => {
            const meeting = await getThisMeeting();
            const handledReports = meeting.reports.filter(item => item.status !== 'Sent');
            handledReportContainer.innerHTML = '';
            handledReports.forEach((report, index) => {
                let tr = `
                <tr>
                    <td>
                        <div class="d-flex align-items-center justify-content-center">
                    ...
                    <td>
                        <div class="text-nowrap d-block text-truncate" style="max-width:150px;" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-html="true" data-bs-original-title="${report.pics.map(pic => pic.name).join(', ')}">
                            ${report.pics.map(pic => pic.name).join(', ')}
                        </div>
                    </td>
                    <td>
                        <div>
                            ${moment(report.deadline).format("DD/MM")}
                        </div>
                    </td>
                    <td class="text-nowrap">
                ...
                if (USER.role === 'admin' || USER.role === 'manager') {
                    ...
                    <td>
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                            ...
                            <li>
                                <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#xoaThuocTinh${report.id}" data-repeater-delete>
                ...
                tr += "</tr>"
                handledReportContainer.insertAdjacentHTML('beforeend', tr);
            })
        }

        renderHandledReports();
