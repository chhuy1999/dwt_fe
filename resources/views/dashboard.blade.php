...
@section('content')
    ...
    @php
        ...
        function totalUserKeyQuantity($targetDetail, $date, $userId)
        {
            $total = 0;
            $targetLogs = $targetDetail->targetLogs;
            foreach ($targetLogs as $targetLog) {
        ...
        function totalAdminKeyQuantity($targetDetail, $date)
        {
            $total = 0;
            $targetLogs = $targetDetail->targetLogs;
            foreach ($targetLogs as $targetLog) {
        ...
        function findUserTargetLogDetail($targetDetail, $date, $userId)
        {
            $targetLogs = $targetDetail->targetLogs;
            foreach ($targetLogs as $targetLog) {
                if (strtotime($targetLog->reportedDate) == strtotime($date)) {
                                ...
                                <div class="card-body">
                                    <div class="table_wrapper">
                                        <div class="table-responsive">
                                            <table id="main_table" class="table table_style-fix m-0 bg-blue-blur" style="width: 100%">
                                                <thead>
                                                    <tr>
                                                        <th colspan="4" class="text-center bg-white position-sticky" style="left:0">Mục tiêu nhiệm vụ tháng
                                                        </th>
                                                        <th colspan="{{ cal_days_in_month(CAL_GREGORIAN, $searchMonth, $searchYear) }}" class="bg-white text-center">Nhật kí công việc
                                                        </th>
                                                    </tr>
                                                    <tr>
                                                            ...
                                                            @for ($i = 0; $i < cal_days_in_month(CAL_GREGORIAN, $searchMonth, $searchYear); $i++)
                                                                ...
                                                                role="button" @endif>
                                                                    <div class="content_table">
                                                                        @if (totalUserKeyQuantity($task, $searchYear . '-' . $searchMonth . '-' . $i + 1, session('user')['id']) > 0)
                                                                            {{ totalUserKeyQuantity($task, $searchYear . '-' . $searchMonth . '-' . $i + 1, session('user')['id']) }}
                                                                        @endif
                                                                    </div>
                                                        ...
                                                        <tbody>
                                                            @foreach ($listAssignedTasks->data as $task)
                                                                <tr>
                                                                    ...
                                                                    role="button" @endif>
                                                                            <div class="content_table">
                                                                                @if (totalAdminKeyQuantity($task, $searchYear . '-' . $searchMonth . '-' . $i + 1) > 0)
                                                                                    {{ totalAdminKeyQuantity($task, $searchYear . '-' . $searchMonth . '-' . $i + 1) }}
                                                                                @endif
                                                                            </div>
                                                                        </td>
                                                                    @endfor
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    <div class="position-relative" data-bs-toggle="tooltip" data-bs-placement="top" title="Thời hạn">
                                        <input value="{{ date('d/m/Y', strtotime($item->deadline)) }}" class="form-control timeSuaVanDe" type="text" name="deadline">
                ...
                <div class="modal-dialog modal-dialog-centered">
                    ...
                    <div class="modal-content">
                        ...
                        <form action="{{ route('targetLog.store', $task->id) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row mb-3">
                                    <div class="kpi-wrapper" id="kpi-wrapper{{ $task->id }}{{ $i }}">
                                        <div class="repeater-datGiaTriKinhDoanh">
                                            ...
                                            <div class="col-md-12">
                                                <div class="d-flex justify-content-start">
                                                    <div role="button" class="fs-4 text-danger" data-repeater-create><i class="bi bi-plus-circle"></i>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="modal-footer">
                                @if (findUserTargetLogDetail($task, $searchYear . '-' . $searchMonth . '-' . $i + 1, session('user')['id']))
                                    <button type="button" class="btn btn-outline-secondary delete-target-log-btn" data-bs-dismiss="modal" data-target-log-id="{{ findUserTargetLogDetail($task, $searchYear . '-' . $searchMonth . '-' . $i + 1, session('user')['id'])->id }}">
                                        Xóa báo cáo
                                    </button>
                                @endif
                                <button type="button" class="btn btn-outline-danger" data-bs-dismiss="modal">Hủy
    ...
    @foreach ($allReportTasks as $reportTask)
        @for ($i = 0; $i < cal_days_in_month(CAL_GREGORIAN, $searchMonth, $searchYear); $i++)
                ...
                <div class="modal-dialog modal-dialog-centered">
                    ...
                    <div class="modal-content">
                        ...
                        <form action="/nhiem-vu-phat-sinh/bao-cao/{{ $reportTask->id }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mb-3 row">
                                    <div class="col-sm-12 d-flex  align-items-center">
                                        <label for="inputPassword" class="col-sm-2 col-form-label">Tên nhiệm
                                            vụ</label>
                                        <div class="col-sm-10">
                                            <input class="form-control" type="text" readonly value="{{ $reportTask->name }}"></input>
                                        </div>
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    <div class="col-sm-12 d-flex  align-items-center">
                                        ...
                                        <div class="col-sm-10">
                                            <textarea class="form-control" name="note" placeholder="Vui lòng nhập nội dung báo cáo vào đây">{{ findReportTaskLog($reportTask, $searchYear . '-' . $searchMonth . '-' . $i + 1)->note ?? '' }}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="mb-3 row">
                            ...
                            <div class="modal-footer">
                                @if (findReportTaskLog($reportTask, $searchYear . '-' . $searchMonth . '-' . $i + 1)->id > 0)
                                    <button type="button" class="btn btn-outline-secondary delete-report-task-btn" data-bs-dismiss="modal" data-report-log-id="{{ findReportTaskLog($reportTask, $searchYear . '-' . $searchMonth . '-' . $i + 1)->id }}">
                                        Xóa báo cáo
                                    </button>
                                @endif
        ...
        @endfor
        <!-- Modal Thông tin nhiệm vụ phat sinh-->
                                        ...
                                        <table class="table table-bordered table-hover">
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div>{{ $reportTask->departement->name ?? '' }}</div>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
...
@section('footer-script')
    ...
    <script>
        const deleteTargetLogBtns = document.querySelectorAll('.delete-target-log-btn');
        console.log(deleteTargetLogBtns);
        for (let i = 0; i < deleteTargetLogBtns.length; i++) {
            deleteTargetLogBtns[i].addEventListener('click', async (e) => {
                e.preventDefault();
                const targetLogId = e.target.dataset.targetLogId;
                const url = API_URL + '/target-log-details/' + targetLogId;
                await fetch(url, {
                    method: "DELETE",
                    headers: {
        ...
        const deleteReportTaskBtns = document.querySelectorAll('.delete-report-task-btn');
        for (let i = 0; i < deleteReportTaskBtns.length; i++) {
            deleteReportTaskBtns[i].addEventListener('click', async (e) => {
                const targetLogId = e.target.dataset.reportLogId;
                console.log(targetLogId);
                const url = API_URL + '/report-tasks-logs/' + targetLogId;
                await fetch(url, {
                    method: "DELETE",
                    headers: {
                        'Authorization': 'Bearer ' + JWT_TOKEN,
